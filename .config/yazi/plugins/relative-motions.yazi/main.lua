...
local render_numbers = ya.sync(function(_, mode)
	...
	-- Suppress Entity:padding for hovered lines since we render it before the number
	local orig_padding = Entity.padding
	Entity.padding = function(self)
		if self._file.is_hovered and self._file.in_current then
			return ""
		end
		return orig_padding(self)
	end
	...
	Current.redraw = function(self)
		...
		for i, f in ipairs(files) do
			...
			local number = Entity:number(i, #self._folder.files, f, hovered_index)

			if f.is_hovered then
				local style_rev = entity:style_rev()
				local padding
				if style_rev then
					padding = ui.Span(th.indicator.padding.open):style(style_rev)
				else
					padding = " "
				end
				entities[#entities + 1] = ui.Line({ padding, number, entity:redraw() })
					:style(entity:style())
			else
				entities[#entities + 1] = ui.Line({ number, entity:redraw() })
					:style(entity:style())
			end
		end
